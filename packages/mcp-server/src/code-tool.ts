...
const prompt = `Runs JavaScript code to interact with the Anyformat API.
...
async function run(client) {
  const response = await client.workflows.run('0686bb97-8c30-70f0-8000-97669e000eb8', { text: 'Invoice #12345\nDate: 2025-01-15\nAmount: $1,250.00\nVendor: Acme Corp' });
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    ANYFORMAT_API_KEY: readEnv('ANYFORMAT_API_KEY') ?? client.apiKey ?? undefined,
    ANYFORMAT_BASE_URL: readEnv('ANYFORMAT_BASE_URL') ?? client.baseURL ?? undefined,
  };
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = '@anyformat/anyformat-ai';
